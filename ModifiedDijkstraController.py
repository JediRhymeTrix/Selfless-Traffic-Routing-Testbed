...
class ModifiedDijkstraPolicy(RouteController):
        ...
        for vehicle in vehicles:
            ...
            while True:
                ...
                if current_edge == vehicle.destination:
                    break
                possible_edges = [
                    edge for edge in unvisited.items() if edge[1]]

                # 2-level sort based on the number of vehicles at each edge and the edge lengths.
                # This is done to avoid congestion at edges.
                current_edge, current_distance = sorted(possible_edges, key=lambda x: (
                    self.connection_info.edge_vehicle_count[x[0]], x[1]))[0]
